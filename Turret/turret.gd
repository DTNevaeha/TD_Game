...
func _physics_process(_delta: float) -> void:
	...
	if target:
		turret_base.look_at(target.global_position, Vector3.UP, true) # True uses the model front
...
func _on_timer_timeout() -> void:
	if target:
		...
		shot.direction = turret_base.global_transform.basis.z # Basis.z makes it so the turret fires in the direction the turret faces.
